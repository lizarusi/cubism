...
cubism.context = function() {
  ...
  context.start = function() {
    ...
    var delay = +stop1 + serverDelay - Date.now() - startFrom;

    // If we're too late for the first prepare event, skip it.
    if (delay < clientDelay) delay += step;

    timeout = setTimeout(function prepare() {
      stop1 = new Date(Math.floor((Date.now() - startFrom - serverDelay) / step) * step);
      start1 = new Date(stop1 - size * step);
  ...
  context.startFrom = function(_){
    startFrom = Date.now() - _;
    return update();
  };
  ...
  function horizon(selection) {
    ...
    selection.each(function(d, i) {
      var that = this,
          ...
          spanTagName = d3.select(that).select(".tagName"),
          max_,
          m = colors_.length >> 1,
          ready;
      ...
      function focus(i) {
        ...
        spanTagName.text(function(d){return metric_.toString()}).style('left', i + 'px');
      }
